...
class Inspector(BaseClass):
    ...
    def get_messages(self, bid):
        ...
        while not self.stop:
            ...
            if message["subject"] == "send":
                ...
                snapshot_latency = (message["preparation_time"] - message["request_time"]).total_seconds()
                self.snapshot_latency_data.append(snapshot_latency)
                total_balance = 0
                log_message = (
                    '\n=============================================='
                    '=============================\n'
    ...
    def run(self):
        ...
        avg_message_size = self.total_message_size / self.message_count
        duration = (time.time() - start_time)
        messages_per_second = self.message_count / duration
        average_storage_size = self.total_storage_size / self.n_global_snapshots
        throughput = self.transaction_count / duration
        average_snapshot_latency = sum(self.snapshot_latency_data) / len(self.snapshot_latency_data)

        print(f"Average message size: {avg_message_size} bytes")
        print(f"Messages per second: {messages_per_second}")
        print(f"Total storage size: {self.total_storage_size} bytes")
        print(f"Average storage size: {average_storage_size} bytes")
        print(f"Throughput: {throughput} transactions per second")
        print(f"Average snapshot latency: {average_snapshot_latency} seconds")